...
def cleanup_old_entries():
    try:
        connection = sqlite3.connect("data_from_all_meters.db")
        cursor = connection.cursor()
        cursor.execute("DELETE FROM DCmeter_89 WHERE date <= DATETIME('now', '-1 years')")
        cursor.execute("DELETE FROM DCmeter_21 WHERE date <= DATETIME('now', '-1 years')")
        cursor.execute("DELETE FROM DCmeter_83 WHERE date <= DATETIME('now', '-1 years')")
        cursor.execute("DELETE FROM DCmeter_57 WHERE date <= DATETIME('now', '-1 years')")
        cursor.execute("DELETE FROM DCmeter_26 WHERE date <= DATETIME('now', '-1 years')")
        cursor.execute("DELETE FROM DCmeter_2_1 WHERE date <= DATETIME('now', '-1 years')")
        cursor.execute("DELETE FROM DCmeter_2_2 WHERE date <= DATETIME('now', '-1 years')")
        cursor.execute("DELETE FROM DCmeter_3_1 WHERE date <= DATETIME('now', '-1 years')")
        cursor.execute("DELETE FROM DCmeter_3_2 WHERE date <= DATETIME('now', '-1 years')")
        cursor.execute("DELETE FROM DCmeter_4_1 WHERE date <= DATETIME('now', '-1 years')")
        cursor.execute("DELETE FROM DCmeter_4_2 WHERE date <= DATETIME('now', '-1 years')")
        cursor.execute("DELETE FROM ACmeter WHERE date <= DATETIME('now', '-1 years')")  
        connection.commit()
        connection.close()
